...
// quick anti-bot: reject if hidden honeypot is filled
function isBot(form) {
  return Boolean(form?.__hp); // add a hidden <input name="__hp" />
}
...
export async function handler(event) {
  ...
  try {
    ...
    if (isBot(form)) {
      return json(200, { ok: true }); // silently succeed to confuse bots
    }
    if (!form.name?.trim() || !emailRe.test(String(form.email || ""))) {
      return json(400, { ok: false, error: "Invalid name/email" });
    }

    // Create a transport per invocation (serverless best practice)
    ...
    return json(500, { ok: false, error: "Mail send failed" });
  }
}
